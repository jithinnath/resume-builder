...
export class Common {
  ...
  public static employmentStack(employments: IEmployment[] | IEducation[]) {
    ...
    employments.forEach(employment => {
      ...
      _stack.push({
        ...
        fontSize: config.subHeader2.fontSize,
        bold: config.subHeader2.bold,
      });
      _stack.push({ text: '' });
  ...
  private static _skillLines(
    level: 'Professional' | 'Fluent' | 'Proficient' | 'Conversant' | 'Intermediate' | 'Beginner'
  ) {
    const LEVEL = {
      Professional: 5,
      Fluent: 5,
